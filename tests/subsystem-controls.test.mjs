...
import { paidCheckoutReadiness, subsystemEnabled, subsystemState, SUBSYSTEM_FLAGS } from "../lib/subsystem-controls.mjs";
...
test("paid checkout cannot open before every product dependency is ready", () => {
  const partial = paidCheckoutReadiness({
  ...
  assert.deepEqual(partial.blockedBy, [SUBSYSTEM_FLAGS.rocketLeaguePublicDetectors]);

  const ready = paidCheckoutReadiness({
    BILLING_CHECKOUT_ENABLED: "true",
    RL_ENGINE_ENABLED: "true",
    RL_PUBLIC_DETECTORS_ENABLED: "true",
    BACKGROUND_PROCESSING_ENABLED: "true",
  });
  assert.deepEqual(ready, { ready: true, blockedBy: [] });
});
