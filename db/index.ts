...
export async function ensureProductSchema(database: D1Database) {
  if (!productSchemaReady) {
    productSchemaReady = database.batch([
      ...
      database.prepare(`CREATE TABLE IF NOT EXISTS replay_batches (
        id integer PRIMARY KEY AUTOINCREMENT NOT NULL,
        public_id text NOT NULL,
        token_hash text NOT NULL,
        analysis_request_id integer NOT NULL,
      ...
      database.prepare("CREATE UNIQUE INDEX IF NOT EXISTS replay_batches_public_id_unique ON replay_batches (public_id)"),
      database.prepare("CREATE UNIQUE INDEX IF NOT EXISTS replay_batches_request_unique ON replay_batches (analysis_request_id)"),
      database.prepare("CREATE INDEX IF NOT EXISTS replay_batches_player_created_idx ON replay_batches (player_id, created_at)"),
      database.prepare("CREATE INDEX IF NOT EXISTS replay_batches_status_idx ON replay_batches (status)"),
      database.prepare(`CREATE TABLE IF NOT EXISTS replay_batch_items (
        id integer PRIMARY KEY AUTOINCREMENT NOT NULL,
        public_id text NOT NULL,
        ...
        candidate_players_json text,
        error_code text,
        error_message text,
        attempts integer DEFAULT 0 NOT NULL,
        created_at text DEFAULT CURRENT_TIMESTAMP NOT NULL,
        updated_at text DEFAULT CURRENT_TIMESTAMP NOT NULL,
        FOREIGN KEY (batch_id) REFERENCES replay_batches(id) ON DELETE CASCADE,
        FOREIGN KEY (upload_session_id) REFERENCES replay_upload_sessions(id)
      )`),
      database.prepare("CREATE UNIQUE INDEX IF NOT EXISTS replay_batch_items_public_id_unique ON replay_batch_items (public_id)"),
      database.prepare("CREATE UNIQUE INDEX IF NOT EXISTS replay_batch_items_upload_unique ON replay_batch_items (upload_session_id)"),
      database.prepare("CREATE UNIQUE INDEX IF NOT EXISTS replay_batch_items_slot_unique ON replay_batch_items (batch_id, valid_slot) WHERE valid_slot IS NOT NULL"),
      database.prepare("CREATE INDEX IF NOT EXISTS replay_batch_items_batch_status_idx ON replay_batch_items (batch_id, status)"),
      database.prepare("CREATE INDEX IF NOT EXISTS replay_batch_items_hash_idx ON replay_batch_items (batch_id, file_sha256)"),
      database.prepare("CREATE INDEX IF NOT EXISTS replay_batch_items_match_idx ON replay_batch_items (batch_id, external_match_id)"),
