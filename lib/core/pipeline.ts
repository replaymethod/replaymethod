  ...
  try {
    ...
    if (result.abstention && result.outputTier === "experimental_early_access") {
      ...
      const completion = await env.DB.batch([
        env.DB.prepare(`UPDATE analysis_usage SET status = 'released', released_at = ?, consumed_at = NULL, updated_at = CURRENT_TIMESTAMP
        ...
        env.DB.prepare(`UPDATE analysis_requests SET status = 'ready', highest_impact_mistake = NULL, why_it_costs = NULL,
          ...
          coach_note = ?, ready_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`).bind(
      ...
      if (!completion[1].meta.changes) return;
      return;
    }
