...
/**
 * Convert every supported persisted/replay timestamp to a canonical UTC ISO
 * value. Invalid or missing values stay null so no caller can accidentally
 * render the literal "Invalid Date".
 */
export function canonicalUtcTimestamp(value) {
  const legacy = legacyUtcTimestamp(value);
...
export function utcTimestampMillis(value) {
  const canonical = canonicalUtcTimestamp(value);
  return canonical ? new Date(canonical).getTime() : null;
}
