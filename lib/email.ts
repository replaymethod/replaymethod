...
export async function sendAnalysisReceived(input: { email: string; game: AnalysisGame; url: string }) {
  return sendEmail(
    ...
    shell(`<p style="color:#61ebb1;font-weight:800;letter-spacing:.08em">MATCH RECEIVED</p><h1 style="font-size:34px;line-height:1.05">Your evidence is in the queue.</h1><p style="color:#a4acc0;line-height:1.7">Replay Method will process the match for one repeated, high-impact decision and stop safely if the available evidence is insufficient.</p><a href="${escapeHtml(input.url)}" style="display:inline-block;margin-top:18px;padding:16px 21px;border-radius:9px;background:#6f54ff;color:white;text-decoration:none;font-weight:800">Track my analysis →</a>`)
  );
}
