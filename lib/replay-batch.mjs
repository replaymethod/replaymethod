...
export const REPLAY_BATCH_AGGREGATION_VERSION = "replay-batch-aggregation@1.0.0";
...
export function aggregateReplayBatch(results, excludedCount = 0) {
  ...
  const recurrenceSummary = primary
    ? `${primary.recurrence} of 10 matches supported this detector; ${10 - primary.recurrence} did not. ${oneOffCount} other signal${oneOffCount === 1 ? " appeared" : "s appeared"} only once and were not promoted to the plan.`
    : `No supported detector recurred in at least 3 of 10 matches. ${oneOffCount} one-off signal${oneOffCount === 1 ? " was" : "s were"} kept out of the coaching plan.`;
  ...
  return {
    ...
    report: primary ? {
      ...
      finding: {
        ...
        evidence,
        limitations: [`Ten-match baseline in one playlist; not a rank benchmark.`, `${10 - primary.recurrence} matches did not support the primary detector.`, ...(firstFinding.limitations || [])].slice(0, 8),
      },
    } : null,
    versions: {
