...
4. The visible story is no longer than `drop replay → add essential context → receive a secure receipt`.
...
The homepage performs these jobs in this order:

1. Problem recognition.
2. Product interaction.
3. Improvement loop.
4. Clearly labeled product proof.
5. Compact trust and direct objection handling.

The first viewport combines jobs 1 and 2. Rocket League is the default, there is no game-selection gate, pricing does not interrupt first value, and navigation is secondary.

## Current implementation truth

| Requirement | Current state | Next evidence required |
| --- | --- | --- |
| One first action | Implemented: the landing page exposes the native `.replay` picker directly | Real-player completion data |
| Three-step red thread | Implemented: Drop replay → Reveal one pattern → Play with one rule | Real-player comprehension data |
| Progressive replay intake | Implemented: file first, minimum context second | Real completion data |
| Honest calibration path | Implemented behind independent fail-closed flag; no analysis is promised | Production migration, flag activation and live smoke |
...
| Two independent qualified reviewers | Stable authenticated reviewer registry, owner qualification, blind initial labels and independent-review gate implemented | Recruit and approve two real qualified reviewers |
...
- Production calibration activation is a separate release action. It never implies public detectors, background coaching or payment activation.
