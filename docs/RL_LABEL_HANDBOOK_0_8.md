# Rocket League 0.8 blind-label handbook

Status: private expert-calibration material for
`rocket-league-expert-labels.v9-mechanics-context-0.6`. It does not authorize
public coaching, threshold changes or holdout access.
...
| `recovery.landing_orientation` | Pitch, roll or heading at surface contact visibly forces correction and delays the next required action compared with a feasible aligned landing. | The car lands on a usable surface alignment and can continue the required job promptly. | Misalignment is visible, but the correct next job, deliberate slowdown or feasible landing alternative is unresolved. |
| `recovery.post_aerial_exit` | After aerial involvement, avoidable landing/heading execution materially extends the commitment before a useful next job is restored. | The player exits the aerial into prompt useful speed and heading without creating a new overcommit. | Re-entry is slow, but the next job may require a wait, controlled possession or unavailable route. |
| `recovery.wall_to_ground` | A feasible wall exit and ground alignment are missed, creating two visible tempo losses before useful re-entry. | The transition preserves useful momentum or deliberately slows for the correct next job. | The wall departure is slow, but route intention, landing surface or required job is outside the retained window. |
...
| `possession.control_space` | With visible time and space, contact speed or angle separates car and ball enough to remove a feasible controlled continuation and gives the opponent first access. | The touch preserves a deliberate controlled continuation or uses the space for a valuable shot, pass or release. | Separation is large, but pressure, intended release, teammate follow-up or a feasible closer touch is unresolved. |
| `possession.wall_control` | A wall touch's approach/contact execution breaks a reachable continuation early and concedes the next controllable action. | The wall touch preserves a second controlled action or creates a deliberate safe release. | Control breaks, but pressure, intended release, wall geometry or the feasible continuation is unresolved. |
