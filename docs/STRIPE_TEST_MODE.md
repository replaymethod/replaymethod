...
- `STRIPE_PRICE_QUARTERLY`: recurring USD Price ID for $15.99 every three months
- `STRIPE_PRICE_MONTHLY`: recurring USD Price ID for $5.99 monthly
...
1. Monthly: USD 5.99, recurring every month.
2. 3-month cycle: USD 15.99, recurring every three months.

Put their `price_…` IDs in the environment variables above. The server accepts
only the `monthly` and `quarterly` plan keys and maps them to these trusted Price
