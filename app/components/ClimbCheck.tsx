...
export default function ClimbCheck() {
  ...
  return <main className="tool-page">
    ...
    <section className="tool-card shell">
      ...
      {game && result && <div className="tool-result"><div className="result-head"><div><span>YOUR STARTING HYPOTHESIS</span><small>{gameNames[game]} · {result.label}</small></div><b>01</b></div><h2>{result.title}</h2><p>{result.explanation}</p><div className="result-focus"><span>YOUR NEXT-QUEUE RULE</span><b>{result.focus}</b></div><div className="result-review"><span>REVIEW THESE THREE MOMENTS</span>{result.review.map((item, index) => <div key={item}><b>0{index + 1}</b><p>{item}</p></div>)}</div><p className="result-honesty">This result is based on your answer, not match data. Replay Method’s planned beta is designed to test the hypothesis against your actual games.</p><ResultSignup game={game} leak={result} /><div className="result-actions"><a href={`/${game}?utm_source=climb_check&utm_campaign=${result.id}`}>See the full {gameNames[game]} method →</a><button onClick={() => setLeakId(null)}>Try another answer</button></div></div>}
    </section>
