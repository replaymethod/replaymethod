...
const configs: Record<GameKey, Config> = {
  general: {
    ...
    lead: "Replay Method is building an evidence-first alternative to stats dashboards and generic AI chat: test one repeated decision, train one rule, then verify the same behavior in later matches.",
  ...
  "rocket-league": {
    ...
    lead: "Upload one representative PC replay to the quality beta. Replay Method parses real match structure and stops unless an evidence detector supports a reliable finding.",
...
export default function Landing({ game = "general" }: { game?: GameKey }) {
  ...
  const intakeLabel = riotRequest ? "Save beta request" : "Start evidence check";
  ...
  return <main>
    <div className="launch-bar"><strong>{riotRequest ? "RIOT ACCESS BETA" : "REPLAY EVIDENCE BETA"}</strong><span>{riotRequest ? "Preserve an opt-in request. Official ingestion is pending." : "Submit one real replay. See the verified outcome."}</span><a href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "launch_bar")}>{intakeLabel} →</a></div>
    <nav className="nav shell"><a className="brand" href="/" aria-label="Replay Method home"><span className="logo">↻</span><span>replay<span>method</span></span></a><div className="nav-links"><a href="/analyze">Beta intake</a><a href="#product">Sample report</a><a href="/guides">Guides</a><a href="#pricing">Pricing</a></div><a className="nav-cta" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "nav_analysis")}>{intakeLabel}</a></nav>

    <section className="hero shell">
      <div className="hero-copy"><div className="eyebrow"><i /> {config.eyebrow}</div><h1>{config.title}<br /><em>{config.accent}</em></h1><p className="lead">{config.lead}</p><GameLinks current={game} placement="hero_picker" />{(game === "general" || game === "rocket-league") ? <><QuickReplayStart placement={game === "general" ? "home_quick_replay" : "rl_quick_replay"} /><div className="hero-quiet-actions"><button className="hero-secondary" onClick={previewProduct}>See a sample report <span>↓</span></button><a href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "hero_full_intake")}>Use the full intake instead →</a></div></> : <div className="hero-actions"><a className="hero-primary" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "hero_analysis")}><span>OFFICIAL ACCESS PENDING · NO CARD</span><b>Save my beta request <i>→</i></b></a><button className="hero-secondary" onClick={previewProduct}>See the product vision <span>↓</span></button></div>}<div className="hero-proof"><span>✓ Real match evidence only</span><span>✓ Stops when unsupported</span><span>✓ No card</span></div><a className="hero-beta-link" href="/climb-check" onClick={() => trackEvent(game, "tool_start", "hero_climb_check")}>No match ready? Run the 60-second Climb Check →</a>
      </div>
    ...
    <section className="free-layer shell"><div className="section-intro"><span className="kicker">VALUE BEFORE PAYMENT</span><h2>Don’t take our word for it.<br />Use the method for free.</h2><p>{riotRequest ? "Use the self-review tools now or preserve an official-access beta request. No unsupported coaching is generated from public profiles." : "Submit a real replay to the evidence-gated beta. A finding appears only when the available evidence supports it."}</p></div><div className="free-layer-grid"><a className="free-card featured" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "free_layer_analysis")}><span>{riotRequest ? "RIOT ACCESS BETA" : "FREE PRODUCT BETA"}</span><h3>{riotRequest ? "Opt-in match request" : "Real replay evidence"}</h3><p>{riotRequest ? "Save your game context and representative link while official Riot account ingestion remains pending." : "Send one original replay. See a finding, an honest quality-gate pause or another verified processing outcome."}</p><b>{intakeLabel} →</b></a><a className="free-card" href="/climb-check" onClick={() => trackEvent(game, "tool_start", "free_layer_check")}><span>60-SECOND SELF CHECK</span><h3>Climb Leak Check</h3><p>No match ready? Find a likely leak class and leave with one immediate focus.</p><b>Run the check →</b></a><a className="free-card" href="/guides"><span>FREE REVIEW SYSTEM</span><h3>Replay & VOD checklists</h3><p>Review the moments that decide games instead of watching an entire match without structure.</p><b>Open the guide library →</b></a></div></section>
    ...
    </div></div><div className="demo-conversion"><div><span>{riotRequest ? "RIOT ACCESS REQUESTS ARE OPEN" : "PRODUCT QUALITY BETA IS OPEN"}</span><b>{riotRequest ? "Preserve your opt-in request without pretending ingestion is live." : "Your first evidence check is free."}</b><p>{riotRequest ? "Explore the intended report loop while official Riot access remains pending." : "See the verified outcome on your own replay before deciding whether the product deserves your money."}</p></div><a href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "demo_analysis")}>{intakeLabel} →</a></div></section>
    ...
    <section className="final"><div className="shell"><span className="kicker">REPLAY. REVEAL. PRACTICE. PROVE.</span><h2>Stop hoping the rank changes.<br />Change what creates the rank.</h2><p className="final-lead">{riotRequest ? "Save an opt-in beta request while official Riot access is pending." : "Submit one real replay and see the evidence-gated outcome free."}</p><a className="final-analysis-cta" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "final_analysis")}>{intakeLabel} →</a><p className="final-alt">Not ready to submit? Join beta updates below.</p><WaitlistForm game={game} config={config} placement="final_form" id="final-signup" /><small>Private beta · No card · No unsupported coaching</small></div></section>
    <footer className="shell"><div className="brand"><span className="logo">↻</span><span>replay<span>method</span></span></div><p>A better way to get better.</p><div className="footer-links"><a href="/analyze">Beta intake</a><a href="/reports">My reports</a><a href="/guides">Guides</a><a href="mailto:contact@replaymethod.xyz">Contact</a><a href="/privacy">Privacy</a><a href="/beta-terms">Beta terms</a></div><small>Not affiliated with Riot Games, Psyonix or Epic Games.</small></footer>
    <div className="mobile-join"><div><span>{riotRequest ? "RIOT ACCESS BETA" : "REPLAY QUALITY BETA"}</span><b>{riotRequest ? "Save a private request" : "Start an evidence check"}</b></div><a href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "mobile_analysis")}>{riotRequest ? "Save" : "Start"} →</a></div>
  </main>;
}
