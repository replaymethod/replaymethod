...
export default function Landing({ game = "general" }: { game?: GameKey }) {
  ...
  return <main>
    ...
    <section className="offer shell" id="pricing"><div className="offer-glow" /><div className="offer-copy"><span className="kicker">A USEFUL FREE COACH, NOT A CRIPPLED DEMO</span><h2>A real diagnosis free. Pay for the improvement loop.</h2><p>Your free report uses the same evidence standard as the paid product. Founding membership is for players who want the coach to remember the focus, compare new matches and prove whether the correction worked.</p><ul><li><i>✓</i> Experience the real product before paying</li><li><i>✓</i> Accuracy is never placed behind a paywall</li><li><i>✓</i> Premium unlocks frequency, memory and proof</li><li><i>✓</i> No card or automatic charge during beta</li></ul></div><div className="offer-tiers"><a className="free-tier" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "pricing_analysis")}><span>FREE BETA · AVAILABLE NOW</span><div><sup>$</sup><b>0</b></div><h3>Your first complete diagnosis</h3><ul><li>One full evidence-backed report</li><li>Your highest-impact repeated mistake</li><li>One next-queue rule and focused plan</li><li>A private report that stays yours</li></ul><strong>Analyze my first match →</strong></a><div className="price-card"><span>FOUNDING 100 · PLANNED</span><div className="price"><sup>$</sup><b>9</b><small>/month</small></div><h3>The ongoing improvement loop</h3><ul><li>Four full analyses each month</li><li>Cross-match pattern memory</li><li>Automatic focus follow-up</li><li>Proof of improvement over time</li></ul><div className="founding-lock">12-month founding price lock while continuously subscribed</div><button onClick={() => scrollTo("final-signup", "pricing_card")}>Reserve founding access →</button><small>No charge today. Billing requires a separate, explicit checkout.</small></div></div></section>
