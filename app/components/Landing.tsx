...
const configs: Record<GameKey, Config> = {
  general: {
    ...
    eyebrow: "AI COACHING BUILT ON YOUR REAL MATCHES",
    title: "Every match leaves evidence.",
    accent: "Use it to rank up.",
    lead: "Replay Method turns your matches into one brutally clear diagnosis, one focused training plan, and proof that the habit is actually changing.",
...
export default function Landing({ game = "general" }: { game?: GameKey }) {
  ...
  return <main>
    ...
    <section className="product-section shell" id="product"><div className="section-intro"><span className="kicker">SEE THE OUTPUT BEFORE YOU JOIN</span><h2>Not another wall of stats.<br />A decision you can use tonight.</h2><p>Replay Method turns evidence from your real matches into a focused correction you can apply in your very next queue.</p></div><div className="product-demo"><div className="demo-sidebar"><span>EXAMPLE PLAYER REPORT</span><strong>{config.label}</strong><div className="demo-tabs" role="tablist" aria-label="Example report views"><button className={demoTab === "diagnosis" ? "active" : ""} onClick={() => setDemoTab("diagnosis")}>01 Diagnosis</button><button className={demoTab === "plan" ? "active" : ""} onClick={() => setDemoTab("plan")}>02 Focus plan</button><button className={demoTab === "verify" ? "active" : ""} onClick={() => setDemoTab("verify")}>03 Verify</button></div><small>Illustrative product preview—not a claimed player result.</small></div><div className="demo-main">
    ...
    <section className="compare"><div className="shell"><div className="section-intro"><span className="kicker">WHY ANOTHER TOOL?</span><h2>Stats describe the loss.<br />A method changes the next game.</h2></div><div className="compare-table"><div className="compare-head"><span>WHAT YOU GET</span><b>MORE GRINDING</b><b>STAT TRACKERS</b><b className="climb-col">REPLAY METHOD</b></div>{[["Shows what happened", "—", "✓", "✓"], ["Explains the repeated decision", "—", "Sometimes", "✓"], ["Gives one personalized focus", "—", "—", "✓"], ["Checks if the habit changed", "—", "—", "✓"]].map(row => <div className="compare-row" key={row[0]}>{row.map((cell, index) => index === 0 ? <span key={cell}>{cell}</span> : <b className={index === 3 ? "climb-col" : ""} key={`${cell}-${index}`}>{cell}</b>)}</div>)}</div></div></section>
    ...
    <section className="offer shell" id="pricing"><div className="offer-glow" /><div className="offer-copy"><span className="kicker">TRY THE METHOD BEFORE YOU PAY</span><h2>Free first. Founding price only if it earns it.</h2><p>The founding beta now accepts real matches. Continue only if the report shows you something genuinely useful.</p><ul><li><i>✓</i> Match evidence, not a personality quiz</li><li><i>✓</i> First analysis free</li><li><i>✓</i> Private report and focused plan</li><li><i>✓</i> No card or charge today</li></ul></div><div className="offer-tiers"><a className="free-tier" href={analysisHref} onClick={() => trackEvent(game, "analysis_start", "pricing_analysis")}><span>AVAILABLE NOW</span><div><sup>$</sup><b>0</b></div><h3>First match analysis</h3><p>One match. One diagnosis. One plan.</p><strong>Submit free →</strong></a><div className="price-card"><span>FIRST 100 PLAYERS</span><div className="price"><sup>$</sup><b>9</b><small>/month</small></div><p>Only if you continue after your free beta analysis</p><button onClick={() => scrollTo("final-signup", "pricing_card")}>Reserve founding access →</button><small>No charge today. Joining the waitlist is not a purchase.</small></div></div></section>
