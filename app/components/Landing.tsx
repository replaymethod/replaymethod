...
export default function Landing({ game = "general", checkoutOpen = false, engineOpen = false }: { game?: GameKey; checkoutOpen?: boolean; engineOpen?: boolean }) {
  ...
  const lead = replayClosed
    ? "The Rocket League evidence engine is finishing its production quality gate. Join the beta list for first access—we are not accepting files or payment until the real replay pipeline is ready."
    : config.lead;
