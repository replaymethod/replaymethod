...
const games: { key: AnalysisGame; mark: string; name: string; proof: string; input: string }[] = [
  ...
  { key: "rocket-league", mark: "RL", name: "Rocket League", proof: "Evidence-gated .replay quality beta", input: "Original PC .replay file" }
];
...
export default function AnalyzeFlow({ initialGame, initialHypothesis }: { initialGame: AnalysisGame | null; initialHypothesis: string }) {
  ...
  return <main className="intake-page">
    ...
    <section className="intake-shell shell">
      <header className="intake-header"><div><span>FREE PRODUCT BETA</span><h1>Bring the evidence.<br /><em>Keep the outcome honest.</em></h1><p>{game === "league" || game === "valorant" ? "Preserve an opt-in Riot beta request while official ingestion is pending. No public profile is treated as authorized match evidence." : "Submit one real replay. The pipeline returns a supported finding or a clear reason it stopped—never invented coaching."}</p></div><aside><b>$0</b><span>{game === "league" || game === "valorant" ? "BETA REQUEST" : "EVIDENCE CHECK"}</span><small>No card · Private status</small></aside></header>
