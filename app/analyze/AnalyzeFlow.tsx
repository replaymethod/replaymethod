...
const games: { key: AnalysisGame; mark: string; name: string; proof: string; input: string }[] = [
  ...
  { key: "rocket-league", mark: "RL", name: "Rocket League", proof: "PC replay + console video beta", input: "Original PC .replay or console gameplay video" }
];
...
const platforms: { key: RocketLeaguePlatform; name: string; evidence: string }[] = [
  { key: "pc", name: "PC", evidence: "Frame-exact .replay" },
  { key: "ps5", name: "PS5", evidence: "Gameplay video or VOD" },
  { key: "xbox", name: "Xbox", evidence: "Gameplay video or VOD" },
  { key: "switch", name: "Switch", evidence: "Clip, capture card or VOD" },
];
...
export default function AnalyzeFlow({ initialGame, initialHypothesis, initialPlatform, engineOpen }: { initialGame: AnalysisGame | null; initialHypothesis: string; initialPlatform: RocketLeaguePlatform | null; engineOpen: boolean }) {
  ...
  const next = () => {
    ...
    if (step === 2 && game === "rocket-league" && platform === "pc" && !engineOpen) return setMessage("PC replay intake opens when the deterministic engine passes its quality gate. Choose a console video lane to submit footage now.");
    if (step === 2 && game === "rocket-league" && platform === "pc" && !replay) return setMessage("Upload the original Rocket League .replay file so the match can be parsed safely.");
    if (step === 2 && game === "rocket-league" && platform !== "pc" && !video && !evidenceUrl.trim()) return setMessage("Upload gameplay video or paste a VOD link for the console video beta.");
  ...
  return <main className="intake-page">
    ...
    <section className="intake-shell shell">
      <header className="intake-header"><div><span>FREE PRODUCT BETA</span><h1>Bring the evidence.<br /><em>Keep the outcome honest.</em></h1><p>{game === "league" || game === "valorant" ? "Preserve an opt-in Riot beta request while official ingestion is pending. No public profile is treated as authorized match evidence." : platform === "pc" ? "Submit one original replay. The pipeline returns a supported finding or a clear reason it stopped—never invented coaching." : "Submit one clear gameplay video or VOD. We score only visible evidence and stop wherever the footage cannot prove the claim."}</p></div><aside><b>$0</b><span>{game === "league" || game === "valorant" ? "BETA REQUEST" : "EVIDENCE CHECK"}</span><small>No card · Private status</small></aside></header>
      ...
      <form className="intake-card" aria-busy={status === "loading"} onSubmit={submit}>
        ...
        {step === 1 && game && <section><button className="intake-back" type="button" onClick={() => setStep(0)}>← CHANGE GAME</button><span className="intake-kicker">PLAYER CONTEXT · {selected?.name.toUpperCase()}</span><h2>Where are you now?</h2>{game === "rocket-league" && <div className="intake-platforms" role="group" aria-label="Rocket League platform">{platforms.map(item => <button type="button" key={item.key} className={platform === item.key ? "active" : ""} aria-pressed={platform === item.key} onClick={() => { setPlatform(item.key); setReplay(null); setVideo(null); setEvidenceUrl(""); setMessage(""); }}><i>{item.key === "pc" ? "PC" : item.key === "ps5" ? "△" : item.key === "xbox" ? "X" : "◫"}</i><span><b>{item.name}</b><small>{item.evidence}{item.key === "pc" && !engineOpen ? " · quality gate closed" : ""}</small></span></button>)}</div>}{initialHypothesis && <div className="intake-hypothesis"><span>HARDSTUCK CHECK CARRIED FORWARD</span><b>{initialHypothesis}</b><small>You selected this as a hypothesis. The match still decides whether it is supported.</small></div>}<div className="field-grid"><label><span>Current rank *</span><input value={currentRank} onChange={e => setCurrentRank(e.target.value)} placeholder="e.g. Gold 2" maxLength={80} /></label><label><span>Target rank</span><input value={targetRank} onChange={e => setTargetRank(e.target.value)} placeholder="e.g. Diamond" maxLength={80} /></label><label className="wide"><span>{contextLabels[game].label} *</span><input value={playerContext} onChange={e => setPlayerContext(e.target.value)} placeholder={contextLabels[game].placeholder} maxLength={160} /></label><label className="wide"><span>What do you want to stop repeating? *</span><textarea value={goal} onChange={e => setGoal(e.target.value)} placeholder="Example: I keep winning early and throwing the lead in mid game." maxLength={500} /></label></div></section>}

        {step === 2 && game && <section><button className="intake-back" type="button" onClick={() => setStep(1)}>← PLAYER CONTEXT</button><span className="intake-kicker">REAL MATCH EVIDENCE</span><h2>Give us something we can prove.</h2><p className="intake-explain">{game === "rocket-league" && platform !== "pc" ? `${platforms.find(item => item.key === platform)?.name} uses video evidence. Upload a clear gameplay clip or paste a VOD; keep the clock, scoreboard and boost meter visible.` : `${selected?.input}. Pick the match that best represents the problem—not your best game.`}</p>{game !== "rocket-league" && <div className="integration-notice"><b>RIOT ACCESS STATUS</b><p>Official opt-in ingestion is awaiting Riot production approval. You may preserve a beta request now, but no automated coaching will be invented from an unverified public profile.</p></div>}{game === "rocket-league" && platform === "pc" && !engineOpen && <div className="integration-notice"><b>PC QUALITY GATE</b><p>The original replay parser is built, but public detector activation is still locked. Choose PS5, Xbox or Switch to submit video evidence now, or join the PC quality beta.</p></div>}<div className="evidence-grid">{game !== "rocket-league" && <label><span>Representative match or profile link</span><input type="url" inputMode="url" value={evidenceUrl} onChange={e => setEvidenceUrl(e.target.value)} placeholder="https://tracker.gg/... or official match page" maxLength={1000} /></label>}{game === "rocket-league" && platform === "pc" && <><label className={`file-drop ${replay ? "has-file" : ""}`}><input ref={replayInputRef} type="file" accept=".replay,application/octet-stream" onChange={e => selectReplay(e.target.files?.[0] || null)} /><i>{replay ? "✓" : "↥"}</i><b>{replay ? replay.name : "Upload the original .replay file"}</b><small>{replay ? `${fileSizeLabel(replay.size)} · validated` : "Maximum 16 MB · Private · Frame-exact evidence"}</small></label>{!replay && <Link className="replay-file-help" href="/replay-upload" target="_blank" rel="noreferrer" aria-label="Can’t find the replay file? Open the 3-step PC guide in a new tab.">Can’t find the file? <span>Open guide · keep this form →</span></Link>}{replay && <div className="replay-value intake-replay-value" role="status" aria-live="polite"><div className="replay-value-head"><span>REPLAY VALIDATED</span><strong>Supported match file recognized.</strong><p>No gameplay finding exists yet. These are real file-readiness checks only.</p></div><div className="replay-value-facts"><div><span>FORMAT</span><b>.replay</b><small>recognized</small></div><div><span>FILE SIZE</span><b>{fileSizeLabel(replay.size)}</b><small>non-empty</small></div><div><span>UPLOAD LIMIT</span><b>PASS</b><small>16 MB maximum</small></div></div><div className="replay-value-plan"><span>HYPOTHESIS TO TEST</span><p>{goal} The parser must first verify the player and match structure; later findings appear only when supported by evidence.</p></div></div>}</>}{game === "rocket-league" && platform !== "pc" && <><label className={`file-drop video-drop ${video ? "has-file" : ""}`}><input ref={videoInputRef} type="file" accept="video/mp4,video/quicktime,video/webm,video/mpeg,.m4v" onChange={e => selectVideo(e.target.files?.[0] || null)} /><i>{video ? "✓" : "▶"}</i><b>{video ? video.name : "Upload gameplay video"}</b><small>{video ? `${fileSizeLabel(video.size)} · video evidence ready` : "MP4, MOV, WebM or MPEG · max 95 MB"}</small></label><div className="evidence-divider"><span>OR</span></div><label><span>Full-match or gameplay VOD link</span><input type="url" inputMode="url" value={evidenceUrl} onChange={e => setEvidenceUrl(e.target.value)} placeholder="https://youtube.com/... or a shareable video link" maxLength={1000} /><small>Use a private/unlisted link only if its sharing settings let the review system open it.</small></label><div className="integration-notice video-honesty"><b>VIDEO EVIDENCE LEVEL</b><p>Video can verify visible decisions and timestamps. It cannot claim hidden player coordinates or frame-exact telemetry. Unsupported moments will be marked insufficient evidence.</p></div></>}<label><span>Anything we should watch for?</span><textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Optional context: controller settings, role, recent change, timestamps or what felt wrong." maxLength={1600} /></label></div></section>}

        {step === 3 && game && <section><button className="intake-back" type="button" onClick={() => setStep(2)}>← MATCH EVIDENCE</button><span className="intake-kicker">PRIVATE DELIVERY</span><h2>Own and recover your {game === "rocket-league" ? "analysis" : "request"}.</h2><div className="delivery-summary"><div><span>GAME</span><b>{selected?.name}{game === "rocket-league" ? ` · ${platforms.find(item => item.key === platform)?.name}` : ""}</b></div><div><span>RANK</span><b>{currentRank}{targetRank ? ` → ${targetRank}` : ""}</b></div><div><span>EVIDENCE</span><b>{replay ? replay.name : video ? video.name : "Private link added"}</b></div></div><div className="field-grid delivery-fields"><label className="wide"><span>Email for private status ownership *</span><input type="email" autoComplete="email" inputMode="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" /></label><label className="check wide"><input type="checkbox" required checked={dataConsent} onChange={e => setDataConsent(e.target.checked)} /><span>I agree that Replay Method may process this match, replay/VOD and my email to deliver the private beta status. <a href="/privacy" target="_blank">Privacy</a></span></label><label className="check wide"><input type="checkbox" checked={updatesConsent} onChange={e => setUpdatesConsent(e.target.checked)} /><span>Also send me product updates and beta-access news. Optional.</span></label></div><div className="honesty-box"><i>{game === "rocket-league" ? platform === "pc" ? "FRAME-EXACT QUALITY BETA" : "CONSOLE VIDEO BETA" : "RIOT ACCESS PREVIEW"}</i><p>{game === "rocket-league" ? platform === "pc" ? "The deterministic parser preserves versioned match evidence and stops instead of guessing when confidence is insufficient." : "The video lane reviews only visible gameplay evidence and timestamps. It never presents video inference as hidden telemetry, and unsupported moments remain unscored." : "Your request and match reference will be preserved, but automated coaching cannot start until the official opt-in Riot integration is approved. No unsupported report will be generated."}</p></div><button className="submit-analysis" disabled={status === "loading"}><span aria-live="polite">{status === "loading" ? "SECURING YOUR MATCH…" : game === "rocket-league" ? platform === "pc" ? "START REPLAY EVIDENCE CHECK — FREE →" : "SUBMIT CONSOLE VIDEO EVIDENCE — FREE →" : "SAVE MY RIOT BETA REQUEST →"}</span></button><small className="submission-note">Your private status link appears immediately. No card or payment details.</small></section>}
