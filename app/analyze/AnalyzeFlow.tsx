...
export default function AnalyzeFlow({ initialGame, initialHypothesis, initialPlatform, engineOpen, videoOpen }: { initialGame: AnalysisGame | null; initialHypothesis: string; initialPlatform: RocketLeaguePlatform | null; engineOpen: boolean; videoOpen: boolean }) {
  ...
  const next = () => {
    ...
    if (step === 2 && game === "rocket-league" && platform === "pc" && !engineOpen) return setMessage("PC replay analysis is still in final quality validation. Join the waitlist for first access.");
    ...
    if (step === 2 && game === "rocket-league" && platform !== "pc" && !videoOpen) return setMessage("Console video analysis is not live yet. Join the console waitlist for first access.");
  ...
  return <main className="intake-page">
    ...
    <section className="intake-shell shell">
      ...
      <form className="intake-card" aria-busy={status === "loading"} onSubmit={submit}>
        ...
        {step > 0 && step < 3 && !(step === 2 && game === "rocket-league" && (platform === "pc" ? !engineOpen : !videoOpen)) && <div className="intake-actions"><button type="button" onClick={next}>CONTINUE <span>→</span></button></div>}
