...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="guide-page">
    ...
    <article className="guide-article shell">
      <header><span>{guide.game} · {guide.readTime}</span><h1>{guide.title}</h1><p>{guide.opening}</p><div><a href={guide.gamePath}>Explore {guide.game} coaching →</a><small>Updated for the current Replay Method beta</small></div></header>
      ...
      <div className="guide-body"><aside><span>15-MINUTE REVIEW</span><ol><li>Pick one close loss</li><li>Review only high-value moments</li><li>Write one repeated pattern</li><li>Queue with one rule</li></ol><a href="/climb-check">Run the free Climb Check →</a></aside><div>{guide.sections.map(section => <section key={section.title}><h2>{section.title}</h2><p>{section.body}</p><ul>{section.bullets.map(item => <li key={item}>{item}</li>)}</ul></section>)}<section className="next-rule"><span>USE THIS NEXT</span><h2>One rule for your next five games</h2><p>{guide.nextRule}</p><a href={`${guide.gamePath}?utm_source=guide&utm_campaign=${guide.slug}#join-beta`}>Join the beta for match-based analysis →</a></section><section className="guide-faq"><span>QUICK ANSWERS</span>{guide.faq.map(item => <div key={item.q}><h2>{item.q}</h2><p>{item.a}</p></div>)}</section></div></div>
    </article>
