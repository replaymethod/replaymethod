...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="guide-page">
    ...
    <CustomerHeader current="method" />
    <article className="guide-article shell">
      <header><span>ROCKET LEAGUE FIELD NOTE · {guide.readTime}</span><h1>Stop watching the goal. Find the decision that caused it.</h1><p>{guide.opening}</p><div><a href={analysisHref}>Analyze my 10 replays →</a><small>Private · No card · Evidence-gated</small></div></header>
      ...
      <div className="guide-body"><aside><span>15-MINUTE REVIEW</span><ol><li>Pick one close loss</li><li>Review only high-value moments</li><li>Write one repeated pattern</li><li>Queue with one rule</li></ol><a href="/analyze">Let Replay Method compare 10 →</a></aside><div>{guide.sections.map(section => <section key={section.title}><h2>{section.title}</h2><p>{section.body}</p><ul>{section.bullets.map(item => <li key={item}>{item}</li>)}</ul></section>)}<section className="next-rule"><span>USE THIS NEXT</span><h2>One rule for your next three matches</h2><p>{guide.nextRule}</p><a href={analysisHref}>Analyze my 10 replays →</a></section><section className="guide-faq"><span>QUICK ANSWERS</span>{guide.faq.map(item => <div key={item.q}><h2>{item.q}</h2><p>{item.a}</p></div>)}</section></div></div>
    </article>
    <CustomerFooter />
  </main>;
}
