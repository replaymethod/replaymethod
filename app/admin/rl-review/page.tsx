...
export default async function RlReviewPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const storedCandidates = await db.select().from(rlReviewCandidates).orderBy(asc(rlReviewCandidates.detectorId), asc(rlReviewCandidates.id));
  ...
  const labels = (await db.select().from(rlReviewLabels).orderBy(asc(rlReviewLabels.createdAt), asc(rlReviewLabels.id)))
    .filter(label => activeCandidateIds.has(label.candidateId));
  ...
  const detectorStats = detectorIds.map(detectorId => {
    ...
    return { detectorId, ...detectorQualitySummary(rows, labels) };
  });
