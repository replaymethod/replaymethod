...
export default async function AdminPage() {
  ...
  const cohortCounts = rankCohorts.map(rank => ({ rank, count: betaReplays.filter(row => row.rankCohort === rank && (row.parsedMode || row.mode) === "2v2" && row.usabilityStatus !== "rejected").length }));
  const nextCohort = [...cohortCounts].sort((a, b) => a.count - b.count)[0];
  const nextReplayNeed = nextCohort ? `Recruit 2v2 ${nextCohort.rank.replaceAll("-", "–")} replays next (${nextCohort.count} currently).` : "Recruit the first consented 2v2 replay.";
