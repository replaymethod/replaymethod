...
import { analysisJobs, analysisRequests, analysisUsage, billingEvents, billingSubscriptions, emailDeliveries, funnelEvents, playerFocuses, rlBetaSubmissions, rlReviewCandidates, waitlist } from "../../db/schema";
...
export default async function AdminPage() {
  ...
  const [leads, events, analyses, jobs, subscriptions, usage, deliveries, focuses, billingEventRows, reviewCandidates, betaReplays] = await Promise.all([
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-table-wrap"><div className="admin-table-head analytics"><span>Player</span><span>Rank / status</span><span>Source</span><span>Received</span><span>Replay</span></div>{betaReplays.length === 0 ? <div className="admin-empty"><b>No consented replays yet.</b><p>Activate the independent intake gate only after storage and reviewer access are ready.</p></div> : betaReplays.map(row => <div className="admin-row analytics" key={row.id}><span><a className="admin-email" href={`mailto:${row.email}`}>{row.email}</a><br /><small>{row.playerName}</small></span><span><i>{row.rankCohort}</i><br /><small>{row.status}</small></span><span>{row.source}<br />{row.campaign && <em>{row.campaign}</em>}</span><span>{new Date(`${row.createdAt}Z`).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" })}</span><span><a href={`/api/admin/rl-beta-submissions/${row.id}/replay`}>Download .replay ↓</a></span></div>)}</section>
