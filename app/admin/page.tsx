...
import { analysisJobs, analysisRequests, funnelEvents, waitlist } from "../../db/schema";
...
export default async function AdminPage() {
  ...
  const [leads, events, analyses, jobs] = await Promise.all([
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-heading"><div><span>MISSION CONTROL</span><h1>{analyses.length} match analyses</h1><p>Monitor automated ingestion, failures, coaching quality and the real improvement funnel from one place.</p></div><Link className="export-button" href="/api/admin/waitlist">Waitlist CSV ↓</Link></section>
