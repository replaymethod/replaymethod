...
export default function ReplayContribution({ intakeOpen, compact = false }: { intakeOpen: boolean; compact?: boolean }) {
  ...
  function acceptFile(file: File | null) {
    ...
    setStatus("idle");
    setMessage("");
    if (!problem && file) trackProductEvent("calibration_start", "rocket-league", "beta_replay_selected");
  }
  ...
  if (!intakeOpen) {
    return <section className={`rl-intake-closed ${compact ? "compact" : ""}`} id="beta-intake" aria-labelledby="rl-intake-title">
      <span>PRIVATE CALIBRATION · NOT OPEN YET</span>
      <h1 id="rl-intake-title">Replay collection is safely closed.</h1>
      <p>The collection path is ready, but uploads remain disabled until private storage and the consented review process are explicitly activated.</p>
  ...
  if (status === "success") {
    return <section className={`rl-intake-success ${compact ? "compact" : ""}`} id="beta-intake" aria-live="polite">
      ...
      <h1>Your match is in the calibration set.</h1>
      <p>Reference <b>{reference}</b>. We will use it only for private Rocket League calibration under the consent you gave. This contribution does not promise a player report.</p>
      ...
      <button type="button" onClick={() => { setReplay(null); setReference(""); setStatus("idle"); }}>Contribute another replay</button>
    </section>;
  }

  return <form className={`rl-intake ${compact ? "compact" : ""}`} id="beta-intake" onSubmit={submit} aria-labelledby="rl-intake-title">
    <header>
      <span>ROCKET LEAGUE · PRIVATE CALIBRATION</span>
      <h1 id="rl-intake-title">{compact ? "Start with the replay." : "Help prove what actually matters in a replay."}</h1>
      <p>{compact ? "Choose one original PC replay. Nothing else appears until the file is accepted." : "Start with one original PC replay. We ask for the minimum context only after the file is accepted."}</p>
    </header>
    ...
    >
      ...
      <button type="button" onClick={() => inputRef.current?.click()}>
        <i>{replay ? "✓" : "↓"}</i>
        <span>{replay ? replay.name : "Choose your .replay"}<small>{replay ? `${formatBytes(replay.size)} · ready` : "or drop it here · max 16 MB"}</small></span>
    ...
    {replay && <section className="rl-intake-context">
      <div className="rl-context-heading"><span>ONE LAST STEP</span><h2>Which player should the evidence follow?</h2><p>No questionnaire. Just enough context to label the replay correctly.</p></div>
      <div className="rl-context-grid">
        <label><span>Email</span><input name="email" type="email" autoComplete="email" inputMode="email" required placeholder="you@email.com" /></label>
      ...
      <div className="rl-intake-consents">
        <label><input name="calibrationConsent" type="checkbox" required /><span>I allow Replay Method to privately store and review this replay to calibrate the Rocket League evidence engine.</span></label>
      ...
      <p className="rl-consent-links"><Link href="/privacy">Privacy</Link> · <Link href="/beta-terms">Calibration terms</Link> · No public profile · No report promised</p>
      <button className="rl-submit" disabled={status === "submitting"}>{status === "submitting" ? "Securing replay…" : "Contribute replay"}<b>→</b></button>
