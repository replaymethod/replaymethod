...
export default function ReportClient({ initial, accessToken, delivery }: { initial: PublicReportData; accessToken: string; delivery: "email" | "link" }) {
  ...
  const momentFeed = [
    ...groupedEvidence.slice(0, 2).map((moment, index) => ({
      ...
      timing: moment.label,
      context: isBatch ? "Example from your ten matches" : "Example from this match",
  ...
  const feedbackQuestions = data.report ? [
    ["observation", isBatch ? "Did the recurring observation fit these ten matches?" : "Did the main observation fit what happened?"],
    ["moment", isBatch ? "Were the cross-match evidence moments useful?" : "Were the highlighted moments correct?"],
  ...
  return <main className="report-page">
    ...
    <section className="report-shell shell">
      <header className="report-top"><div><span>{isBatch ? "PRIVATE · TEN-MATCH BASELINE" : "PRIVATE · VERIFIED REPORT ACCESS"}</span><h1>{data.verifiedFacts?.subjectDisplayName || data.gameLabel}</h1><p>{data.verifiedFacts?.mode || data.processing?.replayContext.mode || "Playlist reading"}{data.verifiedFacts?.rank ? ` · ${data.verifiedFacts.rank} (${data.verifiedFacts.rankProvenance === "verified_replay" ? "verified" : "player-submitted"})` : ""} · {isBatch ? "10 verified matches" : reportDate(data.verifiedFacts?.occurredAt || data.createdAt)}</p></div><i className={data.status}>{data.status === "ready" ? "READY" : stopped ? "PAUSED" : "PROCESSING"}</i></header>
      ...
      {data.status !== "ready" ? <div className={`report-pending ${stopped ? "stopped" : ""}`}><div className="scan-orb"><i /><b>{stopped ? "!" : "↻"}</b></div><span>{stoppedCopy?.kicker || (data.processing?.stageLabel ? "AUTOMATED MATCH ANALYSIS" : "MATCH SECURED")}</span><h2>{stoppedCopy?.title || data.processing?.stageLabel || "Your match is queued."}</h2><p>{stoppedCopy?.body || "Replay Method is reading the submitted match, measuring repeated patterns and selecting one evidence-backed coaching focus."}</p>{identityResolvable && <section className="player-resolution" aria-labelledby="player-resolution-title"><div><span>{data.processing?.replayContext.mode ? `${data.processing.replayContext.mode.toUpperCase()} · PLAYERS FOUND` : "PLAYERS FOUND IN THIS REPLAY"}</span><h3 id="player-resolution-title">Which one is you?</h3><p>Choose your exact player and current playlist rank. The original private replay is reused automatically.</p></div><div className="player-resolution-options" role="radiogroup" aria-label="Players identified in the replay">{data.processing?.candidatePlayers.map(player => <button type="button" role="radio" disabled={!interactive} aria-checked={selectedPlayer === player} className={selectedPlayer === player ? "active" : ""} key={player} onClick={() => { setSelectedPlayer(player); setIdentityRetryState("idle"); }}>{player}</button>)}</div><label className="player-resolution-rank"><span>Your current {data.processing?.replayContext.mode || "playlist"} rank</span><select value={selectedRank} onChange={event => { setSelectedRank(event.target.value); setIdentityRetryState("idle"); }}><option value="">Choose rank</option>{rocketLeagueRanks.map(rank => <option value={rank} key={rank}>{rank}</option>)}</select></label><button className="player-resolution-submit" type="button" disabled={!interactive || !selectedPlayer || !selectedRank || identityRetryState === "saving"} onClick={retryWithPlayer}>{identityRetryState === "saving" ? "Starting…" : identityRetryState === "queued" ? "Analysis queued ✓" : "Analyze this saved replay →"}</button>{identityRetryState === "error" && <p role="alert">The replay could not be queued. Refresh this private report and try again.</p>}</section>}<div className="status-track">{stages.map((stage, index) => <div className={index <= statusIndex && !stopped ? "active" : index < statusIndex ? "complete" : ""} key={stage.key}><i>{index < statusIndex ? "✓" : index + 1}</i><span>{stage.label}</span></div>)}</div><aside>{stopped ? <><b>No fake certainty.</b><span>We stop when the available data cannot support a reliable report.</span></> : delivery === "email" ? <><b>Confirmation sent.</b><span>We’ll send another email when the report is ready.</span></> : <><b>Keep this private link.</b><span>Your report will appear here automatically when it is ready.</span></>}</aside></div> : (data.report || data.earlyAccess || data.performance) && <>
        <section className={`match-in-20 ${data.report ? "has-focus" : "facts-only"}`} id="report-summary" aria-labelledby="report-reveal-title">
          <div className="match-in-20-main">
            <div className="marcel-badges"><span>{isBatch ? "10 REPLAYS · CROSS-MATCH ANALYSIS" : "ONE REPLAY · PRIVATE ANALYSIS"}</span>{data.earlyAccess && <em>{data.earlyAccess.badge}</em>}</div>
            <h2 id="report-reveal-title">{isBatch ? "Your 10 matches, in one clear view." : "Your match in 20 seconds."}</h2>
            <p className="match-result"><b>{isBatch ? `${batchMatches}/10 VERIFIED · ${data.batch?.record.wins}W–${data.batch?.record.losses}L${data.batch?.record.draws ? `–${data.batch.record.draws}D` : ""} · ${data.batch?.confidence.toUpperCase()} CONFIDENCE` : `${matchResult} · ${matchScore}`}</b>{!isBatch && data.performance?.match.overtime && <em>OVERTIME</em>}<span>{playlist}</span></p>
            {isBatch ? <div className="match-in-20-grid">
              <article><small>WHAT KEPT HAPPENING</small><b>{data.report ? `${recurringMatches} of ${batchMatches} matches showed the same pattern` : "No pattern repeated strongly enough"}</b><p>{data.report?.highestImpactMistake || "All ten matches were verified, but the evidence did not support one recurring habit."}</p></article>
              <article><small>WHY IT COSTS YOU</small><b>{data.report?.whyItCosts || "There is not enough evidence to make that claim."}</b><p>{data.report ? "This is the supported consequence across the matched examples below." : "One-off moments remain visible in the facts, but they are not promoted into coaching."}</p></article>
              <article><small>YOUR NEXT 3 MATCHES</small><b>{data.report?.nextQueueRule || "No coaching rule was released"}</b><p>{data.report ? "Use this one cue for three representative matches, then check the replays again." : "Your neutral facts remain available below. A correct abstention does not use the free review allowance."}</p></article>
            </div> : <div className="match-in-20-grid">
          ...
          <aside className="marcel-strength" aria-label="Evidence status and sample size"><small>EVIDENCE STATUS</small><b>{data.report ? (experimental ? "EXPERIMENTAL COACHING" : "SUPPORTED COACHING") : "FACTS ONLY"}</b><span>{isBatch ? "Ten verified matches · cross-match recurrence" : "One replay · within-match evidence"}</span><div><strong>{isBatch ? "10" : "1"}</strong><small>{isBatch ? "REPLAYS" : "REPLAY"}</small></div><p>{isBatch ? `${data.batch?.excludedFiles || 0} excluded file${data.batch?.excludedFiles === 1 ? "" : "s"} did not consume a valid slot. One-off signals are separated from recurring patterns.` : "No rank benchmark, stable-habit claim or calibrated precision is inferred from this single match."}</p></aside>
        </section>

        <nav className="report-section-nav" aria-label="Report sections">
          {(isBatch ? [["report-summary", "Pattern"], ["decision-first", "Why"], ["moments", "Examples"], ["action-plan", "Next 3"], ["performance", "Match facts"], ["confidence", "Confidence"]] : [["report-strength", "What worked"], ["decision-first", "Decision"], ["moments", "Examples"], ["action-plan", "Next 3"], ["performance", "Match facts"], ["confidence", "Confidence"]]).map(([id, label], index) => <button type="button" disabled={!interactive} onClick={() => scrollToSection(id)} key={id}><i>{String(index + 1).padStart(2, "0")}</i>{label}</button>)}
        </nav>

        {!isBatch && <section className="report-strength" id="report-strength"><span>WHAT YOU DID WELL</span><h2>{data.performance?.strength?.title || "No claim released without support."}</h2><p>{data.performance?.strength?.detail || "An honest analysis can withhold a strength claim when this replay does not provide enough direct evidence."}</p>{data.performance?.strength?.limitation && <small>{data.performance.strength.limitation}</small>}</section>}

        {data.report && <section className="report-deep-dive" id="decision-first"><header><span>{isBatch ? "THE HABIT TO CHANGE FIRST" : "THE DECISION TO FIX FIRST"}</span><h2>{data.report.highestImpactMistake}</h2></header><div><article><small>WHY IT COSTS YOU</small><p>{data.report.whyItCosts}</p></article><article><small>WHEN IT MAY BE OKAY</small><p>{data.report.limitations[0] || "The same visible behavior can be correct in another game state; this finding applies only to the replay-linked evidence below."}</p></article></div><aside>{experimental ? "Experimental interpretation—not expert ground truth." : isBatch ? "Supported by a repeated pattern across these matches—not a permanent player trait." : "Supported within this match—not a stable player profile."}</aside></section>}

        {momentFeed.length > 0 && <section className="report-evidence" id="moments"><header><span>{isBatch ? "EXAMPLES FROM YOUR MATCHES" : "MOMENTS THAT PROVE IT"}</span><h2>{isBatch ? "See it happen in different matches." : "See the decision in the match."}</h2><p>Three priority moments first. Open the rest only when the verified evidence supports more.</p></header><div className="moment-list">{strongestMoments.map((moment, index) => <article key={moment.id}><i>{String(index + 1).padStart(2, "0")}</i><div><small>{moment.kind} · {moment.timing}</small><b>{moment.title}</b><p><strong>Context:</strong> {moment.context}</p><p><strong>Evidence:</strong> {moment.observation}</p><p><strong>Likely consequence:</strong> {moment.consequence}</p>{moment.betterAlternative && <p><strong>Better alternative:</strong> {moment.betterAlternative}</p>}<em>{moment.limitation}</em></div></article>)}</div>{momentFeed.length > 3 && <details className="moment-disclosure"><summary>Show all {momentFeed.length} supported moments</summary><div>{momentFeed.map(moment => <article key={`${moment.id}-all`}><small>{moment.kind} · {moment.timing}</small><b>{moment.title}</b><p><strong>Context:</strong> {moment.context}</p><p><strong>Evidence:</strong> {moment.observation}</p><p><strong>Likely consequence:</strong> {moment.consequence}</p>{moment.betterAlternative && <p><strong>Better alternative:</strong> {moment.betterAlternative}</p>}<em>{moment.limitation}</em></article>)}</div></details>}</section>}

        {data.report ? <section className="one-focus-plan" id="action-plan"><header><span>YOUR NEXT 3 MATCHES</span><h2>{data.report.nextQueueRule}</h2><p>Use one cue for the next three representative {playlist} matches. Leave everything else alone for now.</p></header><div className="one-focus-grid"><article><small>IF–THEN RULE</small><b>{data.report.nextQueueRule}</b></article><article><small>5–10 MINUTE DRILL</small><b>{data.report.practicePlan[0] || "No drill was defensibly linked to this finding."}</b></article><article><small>WHAT TO NOTICE</small><b>{data.report.practicePlan[1] || "Notice the same game state before the decision—not only the final outcome."}</b></article><article><small>WHAT THE NEXT REPLAY CHECKS</small><b>{data.report.practicePlan[2] || data.report.coachNote || "Whether the same replay-linked decision appears again under comparable conditions."}</b></article></div>{data.report.coachNote && <aside><span>COACH NOTE</span><p>{data.report.coachNote}</p></aside>}</section> : <section className="one-focus-plan abstained" id="decision-first"><header><span>HONEST RESULT</span><h2>{isBatch ? "No repeated habit was clear enough to coach." : "No coaching plan was released from this replay."}</h2><p>{isBatch ? "All ten matches were verified. Your match facts and examples remain below, but Replay Method did not turn one-off moments into a made-up habit. This result does not use the free review allowance." : "The verified review remains the complete result. Replay Method did not turn neutral measurements into a fake mistake or generic drill."}</p></header><span id="action-plan" /></section>}

        {data.performance ? <section className="performance-review" id="performance"><header><span>{isBatch ? "WHAT WE COULD MEASURE" : "FULL MATCH STATS"}</span><h2>{isBatch ? "The neutral facts across all ten matches." : "What happened—and what each measure can tell you."}</h2><p>{data.performance.metrics.length} explained measures across {performanceGroups.length} applicable categories. Open a metric for meaning and limits.</p></header><div className="performance-groups">{performanceGroups.map(([category, metrics]) => <article className="performance-category" key={category}><h3>{performanceCategoryLabels[category] || category.replaceAll("_", " ")}</h3><div>{metrics.map(metric => <details className={`performance-metric ${metric.status}`} key={metric.id}><summary><span><small>{metric.kind.replaceAll("_", " ")}</small><b>{metric.label}</b></span><strong>{metric.displayValue}</strong></summary><div><p><b>What happened:</b> {metric.whatHappened}</p><p><b>Why it matters:</b> {metric.whyItMatters}</p><p><b>What it does not prove:</b> {metric.limitation}</p><small>SOURCE · {metric.source} · {metric.version}{metric.sampleCount != null ? ` · N=${metric.sampleCount}` : ""}</small></div></details>)}</div></article>)}</div></section> : <section className="performance-review unavailable" id="performance"><header><span>FULL MATCH STATS · UNAVAILABLE</span><h2>Detailed match measures were not available for this replay.</h2><p>The verified moments and report remain intact. Replay Method did not estimate or fill in missing metrics.</p></header></section>}

        <section className="report-confidence" id="confidence">
          <header><span>HOW SURE IS THIS?</span><h2>What is solid, what is interpreted, and what these matches cannot prove.</h2><p>The plain answer comes first. Technical versions and internal checks stay inside Advanced details.</p></header>
          <div className="confidence-status">
            <article><i>✓</i><div><b>Verified replay facts</b><p>Player, playlist, timestamps and direct telemetry come from the original {isBatch ? "ten files" : "file"}.</p></div></article>
            <article className={data.performance ? undefined : "abstained"}><i>{data.performance ? "≈" : "—"}</i><div><b>{data.performance ? "Derived match measures" : "Detailed measures unavailable"}</b><p>{data.performance ? "Calculated values describe this match; they do not establish a long-term habit." : "No missing match measures were estimated or presented as facts."}</p></div></article>
            <article className={data.report ? "experimental" : "abstained"}><i>{data.report ? "β" : "—"}</i><div><b>{data.report ? "Early Access coaching" : "Coaching withheld"}</b><p>{data.report ? (isBatch ? "A pattern repeated across these matches and was strong enough to show, while expert validation continues." : "This match supported one useful interpretation, while expert validation continues.") : "The available evidence did not support a defensible coaching claim."}</p></div></article>
          </div>
          {factEntries.length > 0 && <div className="verified-match-facts"><header><span>VERIFIED MATCH CONTEXT</span><h3>What the replay itself established.</h3></header><div>{factEntries.map(([label, value]) => <article key={label}><small>{label}</small><b title={value}>{value}</b></article>)}</div></div>}
          <details className="report-method"><summary><span>Advanced details</span><small>Technical evidence, versions and detector decisions</small></summary><div className="report-method-grid"><div><span>METHOD</span><h2>Traceable coaching, not a black box.</h2><p>{data.earlyAccess ? "Verified facts come from the replay parser. Any coaching insight is a separately marked deterministic Early Access interpretation; no language model may add gameplay facts, and expert validation is still in progress." : data.report?.analysisSource === "automated" ? "This report was generated from versioned structured findings. The language layer can explain and prioritize them, but it cannot create new gameplay facts." : "This beta report was quality-reviewed. Automated engine metadata appears for reports produced by the structured pipeline."}</p></div><aside><b>{data.processing?.versions.parser || "Parser unavailable"}</b><span>Parser</span><b>{data.processing?.versions.detector || "Quality-reviewed beta"}</b><span>Detector</span><b>{data.processing?.versions.schema || "Legacy report schema"}</b><span>Schema</span><b>{data.verifiedFacts?.gameVersion || "Unavailable"}</b><span>Game version</span></aside>{data.earlyAccess && <div className="method-decisions"><b>LOCAL DETECTOR DECISIONS</b>{data.earlyAccess.assessments.map(item => <article className={item.status} key={item.detectorId}><strong>{item.detectorId}</strong><span>{item.status === "experimental_insight" ? `EXPERIMENTAL · ${item.reason}` : `ABSTAINED · ${item.reason}`}</span></article>)}</div>}<div className="report-limitations"><b>KNOWN LIMITATIONS</b>{data.report?.limitations.length ? <ul>{data.report.limitations.map(item => <li key={item}>{item}</li>)}</ul> : <p>{data.earlyAccess ? "No coaching finding passed for this replay. Formal detector validation remains pending." : "No additional detector-specific limitations were recorded for this finding."}</p>}</div></div></details>
          {data.earlyAccess && <aside className="early-access-compact"><span>{data.earlyAccess.badge}</span><p>{data.earlyAccess.body}</p><small>Expert validation is still in progress · This individual report has not been human-reviewed.</small></aside>}
        </section>

        <section className="report-feedback" id="feedback"><span>{data.earlyAccess ? "EARLY ACCESS PRODUCT FEEDBACK" : "REPORT FEEDBACK"}</span><h2>{data.report ? "Help us improve the report experience." : "Help us improve the facts-only experience."}</h2><p className="feedback-boundary">These answers are product feedback only. They never change what the replay itself proved.</p>{feedbackState === "saved" ? <div className="feedback-saved" role="status"><i>✓</i><b>Feedback saved separately from replay evidence.</b></div> : <><div className="feedback-questions">{feedbackQuestions.map(([key, question]) => <fieldset key={key}><legend>{question}</legend><div>{[["yes", "Yes"], ["not_sure", "Not sure"], ["no", "No"]].map(([value, label]) => <button type="button" aria-pressed={feedbackSignals[key] === value} className={feedbackSignals[key] === value ? "active" : ""} key={value} onClick={() => setFeedbackSignals(previous => ({ ...previous, [key]: value }))}>{label}</button>)}</div></fieldset>)}</div><textarea aria-label="Optional report feedback" value={feedbackText} onChange={e => setFeedbackText(e.target.value)} placeholder="What was missing, wrong, or unclear?" maxLength={1000} /><label><input type="checkbox" checked={caseStudyConsent} onChange={e => setCaseStudyConsent(e.target.checked)} /><span>You may quote this feedback anonymously as an Early Access product review.</span></label><button type="button" className="save-feedback" disabled={!Object.values(feedbackSignals).some(Boolean) || feedbackState === "saving"} onClick={saveFeedback}>{feedbackState === "saving" ? "Saving…" : "Save feedback"}</button>{feedbackState === "error" && <p role="alert">Could not save feedback. Try again.</p>}</>}</section>

        <aside className="premium-bridge"><span>PREMIUM · COMING LATER · NOT FOR SALE</span><h2>Free: find the pattern. Premium: prove you fixed it.</h2><p>A future Premium will compare up to 35 representative replays per week and measure change over time. The complete ten-match report above is free; there is no checkout or locked evidence here.</p></aside>
      </>}
    </section>
  </main>;
}
