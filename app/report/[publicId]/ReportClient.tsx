...
export default function ReportClient({ initial, delivery }: { initial: PublicReportData; delivery: "email" | "link" }) {
  ...
  const evidence = data.report?.evidenceDetails.length
    ? data.report.evidenceDetails.map(item => ({
      label: item.round != null ? `Round ${item.round}` : item.timestamp != null ? `${Math.floor(item.timestamp / 60)}:${String(Math.floor(item.timestamp % 60)).padStart(2, "0")}` : item.label,
  ...
  return <main className="report-page">
    <nav className="tool-nav shell"><Link className="brand" href="/"><span className="logo">↻</span><span>replay<span>method</span></span></Link><div><Link href="/reports">My reports</Link><button onClick={copyLink}>{copied ? "Copied ✓" : "Copy private link"}</button></div></nav>
    <section className="report-shell shell">
      ...
      {data.status !== "ready" ? <div className={`report-pending ${stopped ? "stopped" : ""}`}><div className="scan-orb"><i /><b>{stopped ? "!" : "↻"}</b></div><span>{stoppedCopy?.kicker || (data.processing?.stageLabel ? "AUTOMATED MATCH ANALYSIS" : "MATCH SECURED")}</span><h2>{stoppedCopy?.title || data.processing?.stageLabel || "Your match is queued."}</h2><p>{stoppedCopy?.body || "Replay Method is reading the submitted match, measuring repeated patterns and selecting one evidence-backed coaching focus."}</p><div className="status-track">{stages.map((stage, index) => <div className={index <= statusIndex && !stopped ? "active" : index < statusIndex ? "complete" : ""} key={stage.key}><i>{index < statusIndex ? "✓" : index + 1}</i><span>{stage.label}</span></div>)}</div><aside>{stopped ? <><b>No fake certainty.</b><span>We stop when the available data cannot support a reliable report.</span></> : delivery === "email" ? <><b>Confirmation sent.</b><span>We’ll send another email when the report is ready.</span></> : <><b>Keep this private link.</b><span>Your report will appear here automatically when it is ready.</span></>}</aside></div> : data.report && <>
        <div className="report-hero"><div><span>YOUR PRIMARY LEAK</span><h2>{data.report.highestImpactMistake}</h2><p>{data.report.whyItCosts}</p></div><aside><small>METHOD SIGNAL</small><b>{data.report.confidenceLabel ? `${data.report.confidenceLabel.toUpperCase()} CONFIDENCE` : "QUALITY REVIEWED"}</b><span>{confidence == null ? "Evidence checked before publishing" : `${confidence}% detector confidence · ${data.report.analysisSource === "automated" ? "automated" : "reviewed"}`}</span></aside></div>
        ...
        <section className="verify-next"><div><span>04 · PROVE</span><h2>Your diagnosis is complete. The improvement loop checks whether this decision actually changes.</h2><p>A paid plan remembers this focus, compares it across your next matches and shows when the underlying habit improves—even before your rank catches up.</p></div><aside><span>RECOMMENDED · 3-MONTH CYCLE</span><b>$27<small>/3 months</small></b><ul><li>$9/month effective</li><li>4 analyses every 30 days</li><li>Save $9 vs monthly</li></ul><Link href="/#pricing" onClick={trackUpgradeInterest}>Compare paid plans →</Link><small>Renews every 3 months · cancel before renewal</small></aside></section>
        ...
        <section className="report-feedback"><span>VERIFIED BETA FEEDBACK</span><h2>Did this show you something useful?</h2>{feedbackState === "saved" ? <div className="feedback-saved"><i>✓</i><b>Feedback saved. Thank you for helping build the method.</b></div> : <><div className="score-row" aria-label="Rate this report from 1 to 5">{[1,2,3,4,5].map(score => <button className={feedbackScore === score ? "active" : ""} key={score} onClick={() => setFeedbackScore(score)}>{score}<small>{score === 1 ? "Not useful" : score === 5 ? "Very useful" : ""}</small></button>)}</div><textarea value={feedbackText} onChange={e => setFeedbackText(e.target.value)} placeholder="What was useful—or what was missing?" maxLength={1000} /><label><input type="checkbox" checked={caseStudyConsent} onChange={e => setCaseStudyConsent(e.target.checked)} /><span>You may quote this feedback anonymously as a verified beta review.</span></label><button className="save-feedback" disabled={!feedbackScore || feedbackState === "saving"} onClick={saveFeedback}>{feedbackState === "saving" ? "Saving…" : "Save feedback"}</button>{feedbackState === "error" && <p>Could not save feedback. Try again.</p>}</>}</section>
      </>}
    </section>
  </main>;
}
